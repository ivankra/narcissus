...
var hostGlobal = (typeof globalThis === 'object' ? globalThis : require('./global'));
...
function wrapNative(name, val) {
    ...
    return new Proxy(function() {}, {
      ...
      apply: function(target, thisArg, args) { return val.apply(hostGlobal, args); },
      construct: function(target, args, newTarget) { return applyNew(val, args); }
    });
}

var hostHandler = definitions.blacklistHandler(hostGlobal, options.hiddenHostGlobals);
var hostHandlerGet = hostHandler.get;
hostHandler.get = function(target, name, receiver) {
    return wrapNative(name, hostHandlerGet(target, name, receiver));
};
//var hostProxy = new Proxy({}, hostHandler);
var hostProxy = hostGlobal;  // TODO fix hostProxy
                ...
                for (;;) {                  // this loop exits switch_loop
                    if (t.statements.children.length) {
                        try {
                            ...
                            if (e === BREAK_SIGNAL && x.target === n) break switch_loop;
                            else throw e;
                        }
                    }
                    if (++i === j)
      ...
      case WHILE:
        while (!n.condition || getValue(execute(n.condition, x))) {
            try {
                ...
                if (e === BREAK_SIGNAL && x.target === n) break;
                else if (e === CONTINUE_SIGNAL && x.target === n) {} // Must run the update expression.
                else throw e;
            }
      ...
      case FOR_IN:
        ...
        for (i = 0, j = a.length; i < j; i++) {
            ...
            try {
                ...
                if (e === BREAK_SIGNAL && x.target === n) break;
                else if (e === CONTINUE_SIGNAL && x.target === n) continue;
                else throw e;
            }
        }
        break;
...
function newModule(mod, scope) {
    ...
    function getExportValue(target, name, receiver) {
...
function newFunction(n, x) {
    ...
    var p = new Proxy(function() {}, {
      ...handler,
      apply: function(target, thisArg, args) { return fint.call(p, thisArg, args, x); },
      construct: function(target, args, newTarget) { return fint.construct(p, args, x); }
    });
    functionInternals.set(p, fint);
    var proto = {};
    definitions.defineProperty(p, "prototype", proto, true, false, true);
    // TODO: fix property lookups via prototype in interpreted code
...
var FIp = FunctionInternals.prototype = {
    call: function(f, t, a, x) {
        ...
        try {
            ...
            if (e === RETURN_SIGNAL) return x2.result;
            else throw e;
        }
        return undefined;
    },
...
function repl() {
    ...
    for (;;) {
        ...
        try {
          ...
          else if (e === CANCEL_SIGNAL) continue;
